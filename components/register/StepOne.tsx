...
export default function StepOne({
  ...
  /* ── Borde y fondo dinámico del campo email según su estado ── */
  ...
  const renderEmailStatusIcon = () => {
    switch (emailStatus) {
      case "checking":
        return (
          <ActivityIndicator size="small" color={mutedForeground} style={styles.emailStatusIcon} />
        );
      case "available":
        return (
          <Ionicons name="checkmark-circle" size={20} color={successColor} style={styles.emailStatusIcon} />
        );
      case "taken":
        return (
          <Ionicons name="close-circle" size={20} color={errorColor} style={styles.emailStatusIcon} />
        );
      case "error":
        return (
          <Ionicons name="alert-circle" size={20} color={warningColor} style={styles.emailStatusIcon} />
        );
      default:
        return null;
    }
  ...
  const renderEmailHint = () => {
    if (emailStatus === "available") {
      return <Text style={[styles.hintText, { color: successColor }]}>Correo disponible</Text>;
    }
    if (emailStatus === "taken") {
      return <Text style={[styles.hintText, { color: errorColor }]}>Este correo ya está registrado</Text>;
    }
  ...
  const checkEmailExists = async (email: string) => {
    ...
    try {
      ...
      // Error de red o HTTP → avisamos pero no bloqueamos el registro
      setEmailStatus("error");
      onEmailTakenChange(false);
    }
  };

  const handleEmailBlur   = () => checkEmailExists(formData.email);
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View style={styles.fullnameContainer}>
        ...
        <View style={styles.inputContainer}>
          <Text style={[styles.inputLabel, { color: foreground }]}>Apellido</Text>
          <TextInput
            ref={apellidoRef}
      ...
      <View style={styles.fieldContainer}>
        <Text style={[styles.inputLabel, { color: foreground }]}>Correo electrónico</Text>

        <View
          style={[
            styles.emailContainer,
            { borderColor: emailBorderColor(), backgroundColor: emailBgColor() },
          ]}
        >
          <TextInput
            ref={emailRef}
            ...
            onSubmitEditing={() => {
              if (emailStatus !== "checking") passwordRef.current?.focus();
            }}
            returnKeyType="next"
          />
      ...
      {/* ── Contraseña ── */}
      <View style={styles.fieldContainer}>
        <Text style={[styles.inputLabel, { color: foreground }]}>Contraseña</Text>
      ...
      <View style={styles.fieldContainer}>
        <Text style={[styles.inputLabel, { color: foreground }]}>Confirmar contraseña</Text>
